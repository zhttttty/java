...
public class HeartRateController extends BaseController {
	...
	@ResponseBody
	public Result download(HeartRate heartRate) {
		String uId = getUId();
		if (uId != null) {
			if (heartRate.getDates() != null) {
				Map<String, Object> map = new HashMap<String, Object>();
				heartRate.setUserId(uId);
				// 查询起始日期的所有数据
				List<HeartRate> list = heartRateService.download(heartRate);
				if (list.size() > 0) {
					// 获取数据成功
					map.put("res", 1);
					map.put("rows", list);
	...
	@ResponseBody
	public Result downloadRange(HeartRate heartRate, Integer plusType) {
		String uId = getUId();
		if (uId != null) {
			Long date = heartRate.getDates();
			if ((date != null) && (((plusType == null || plusType == 0)) || (plusType >= 11 && plusType <= 13))) {
				Map<String, Object> map = new HashMap<String, Object>();
				List<HeartRate> list = null;
				if (plusType == null || plusType == 0) {
					// 查询日期范围数据
					list = heartRateService.findByDays(uId, Utils.days(date, 0, plusType),
							Utils.days(date, 0, plusType));
				} else {
					// 查询日期范围数据
					list = heartRateService.findByDays(uId, Utils.days(date, 0, plusType - 10),
							Utils.days(date, 0, plusType));
				}
				if (list.size() > 0) {
					// 获取数据成功
	...
	public Result upload(String data) {
		...
		if (uId != null) {
			if (StringUtils.isNotBlank(data)) {
				Map<String, Object> map = new HashMap<String, Object>();
				// 上传data 返回res
				map.put("res", heartRateService.upload(uId, data));
				return Result.ok(map);
			} else {
				return Result.build("111", "参数校验不通过");
			}
